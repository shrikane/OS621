...
global:register_name(er_calculate, spawn(twovm, calculate, [])),
...
initProc(Limit,N,Topology,Function,Frag) ->  
    ...
    %abhi's code
    NumberNodes = length(nodes()) + 1,
    Evaluation = (Limit rem NumberNodes),
    %io:format("Evaluation is ~p ~n", [Evaluation]),
    %Processname = list_to_atom(string:concat( "P_" ,integer_to_list(Limit))),
	%based on modulo arithmetic, i just assigned the processes accordingly so that it becomes 
	%a 50-50 split. I am making some small error here, I am not sure what it is. If someone could check, 
	%and let me know that would be great. 
     if
     Evaluation == 0-> global:register_name(Processname ,spawn_link(twovm , threadoperation , [Route,Lines,FragId]));
     true -> global:register_name(Processname ,spawn_link(lists:nth(Evaluation, nodes()), twovm, threadoperation , [Route,Lines,FragId]))
     end,
       
     %abhi's code ends here
...
startprocessing(Pidlist, Function) ->
	...
	%lists:foreach(fun(Pid) -> global:send(Pid , {timer, Function}) end, Pidlist).	       		      
	...
	receive 
		...
		{Pid, request, Function, Sum, Size} ->
			%io:format("~p Got a request from ~p to calculate ~p. Received sum is ~p. Received size is ~p ~n", [self(),Pid, Function, Sum, Size]),
	      		global:send(Pid , {self(),reply, Function, get("localsum"), get("localsize")}),% send a reply message with local values	
			global:send(er_calculate , {self(), Function, get("localsum"), Sum, get("localsize"), Size}),
			threadoperation(Pids, Mydata,SegId);	
		...
		{Pid, request, Function, Value} ->
			...
			case Function of
				...
				%io:format("er_calc ~p  ~n", [whereis(er_calculate)]),
					...
					global:send( whereis(er_calculate) , {self(), Function, get("Function"), Value}),
					global:send(Pid , {self(), reply, Function, get("Function")});
				max ->
					global:send(er_calculate , {self(), Function, get("Function"), Value}),
					global:send(Pid , {self(), reply, Function, get("Function")})
			end,
		...
		{timer, Function} ->
			...
			global:send(self() , {calculate, Function}),
